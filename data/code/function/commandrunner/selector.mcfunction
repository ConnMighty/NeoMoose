...
$execute if items block ~ ~-1 ~ container.15 rabbit_foot run execute as @e[tag=!PLOT,tag=!SPAWN,distance=..2500,type=$(selector_input_string)] at @s run return run function code:commandrunner/commands/$(function) with storage runner "$(UUID)"
