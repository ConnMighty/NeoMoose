...
$execute store result storage runner "$(UUID)".runner_id int 1 run scoreboard players get @s runner_id

$execute if items block ~ ~-1 ~ container.15 magenta_glazed_terracotta run execute as @e[tag=!PLOT,tag=!SPAWN,distance=..2500,scores={temp_id=$(context_input)}] at @s run return run function code:commandrunner/$(path)$(function) with storage runner "$(UUID)"
